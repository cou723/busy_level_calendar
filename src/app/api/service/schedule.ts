...
export const schedule = {
  async create(userId: User["id"], schedule: ScheduleWithoutDefault): Promise<NextResponse<Schedule | ErrorResponse>> {
    ...
    if (result.err) return makeErrorResponse(500, "scheduleの作成に失敗しました: " + result.val.message);
  ...
  async update(
    ...
    if (result.err) return makeErrorResponse(500, "scheduleの更新に失敗しました: " + result.val.message);
  ...
  async delete(userId: User["id"], id: Schedule["id"]): Promise<NextResponse<void | ErrorResponse>> {
    ...
    if (result.err) return makeErrorResponse(500, "scheduleの削除に失敗しました: " + result.val.message);
